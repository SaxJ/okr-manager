...
import           Yesod.Auth.OpenId              ( authOpenId
...
instance Yesod App where
  ...
  defaultLayout widget = do
    ...
    let menuItems =
          [ NavbarLeft $ MenuItem { menuItemLabel          = "Home"
                                  , menuItemRoute          = HomeR
                                  , menuItemAccessCallback = True
                                  }
          , NavbarLeft $ MenuItem { menuItemLabel          = "OKRs"
                                  , menuItemRoute          = OkrsR
          ...
          , NavbarRight $ MenuItem { menuItemLabel          = "Login"
                                   , menuItemRoute          = AuthR LoginR
                                   , menuItemAccessCallback = isNothing muser
                                   }
          , NavbarRight $ MenuItem { menuItemLabel          = "Logout"
  ...
  isAuthorized (CheckInR    _)     _ = isAuthenticated
...
instance YesodAuth App where
  ...
  authenticate creds = liftHandler $ runDB $ do
    ...
    case x of
      ...
      Nothing -> Authenticated <$> insert User { userIdent    = credsIdent creds
  ...
  authPlugins app = googleAuth ++ [authOpenId Claimed []] ++ extraAuthPlugins
   where
    extraAuthPlugins = [ authDummy | appAuthDummyLogin $ appSettings app ]
    googleClientId   = "foo"
    googleSecret     = "bar"
    googleAuth       = [oauth2Google googleClientId googleSecret]
...
isAuthedAsAdmin = do
  ...
  return $ case mUser of
    Nothing -> Unauthorized "You must login to access this page"
    Just (Entity _ user) ->
      if userAdmin user then Authorized else Unauthorized "You must be admin"
